...
def _get_upload_info(driver, ord_no, suffix=".pdf", cur_loc_id="343"):
    """
    调用 GetUploadInfo，获取 FTPS 上传路径
    签名: GetUploadInfo(ordNo, suffix, curLocID)
    返回 JSON: {"serverPath": "ftps://...", "path": "dhcpeftp/..."}
    """
    import json
    raw = _csp_call(driver, "GetUploadInfo", ord_no, suffix, cur_loc_id)
    try:
        return json.loads(raw)
    except Exception:
        return {"path": f"dhcpeftp/images/{ord_no}"}


def _ftps_upload_jpg(jpg_path, ord_no):
    """
    将本地 JPG 文件上传到 FTPS /dhcpeftp/images/{ord_no}/ 目录
    文件名保持 {ord_no}_{page}.jpg
    """
    ...
    remote_name = os.path.basename(jpg_path)
    file_size = os.path.getsize(jpg_path)
    if file_size == 0:
        raise RuntimeError(f"JPG 文件为空: {jpg_path}")

    with open(jpg_path, 'rb') as f:
        resp = ftps.storbinary(f"STOR {remote_name}", f)

    if not resp.startswith("226"):
        raise RuntimeError(f"FTPS上传JPG失败: {resp}")
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        hdr.setStyleSheet("background: white; border-bottom: 1px solid #F0F0F0;")
        hl = QHBoxLayout(hdr)
        hl.setContentsMargins(24, 0, 24, 0)
        t = QLabel("体检报告批量上传")
        t.setStyleSheet("font-size: 18px; font-weight: 700; color: #1D1D1F;")
        hl.addWidget(t)
        hl.addStretch()
        self.lbl_total = QLabel("今日客户数: --")
        self.lbl_total.setStyleSheet(
            "background: #E8F4FF; color: #0066CC; padding: 4px 14px; "
            "border-radius: 20px; border: 1px solid #CCE4FF; "
    ...
    def start_scan(self):
        self.lbl_total.setText("今日客户数: 扫描中...")
    ...
    def _on_scan_progress(self, msg, pct):
        self.lbl_total.setText(f"今日客户数: 扫描中... {msg}")

    def _on_scan_finished(self, patients):
        self.patients = patients
        self.lbl_total.setText(f"今日客户数: {len(patients)}")
    ...
    def _fetch_and_upload(self, user_id_input):
        """获取医嘱号阶段：复用 driver 调用 GetBaseInfo"""
        from selenium.webdriver.chrome.options import Options
        all_reports = [(p, rep) for p in self.patients for rep in p.reports]
    ...
    def _do_upload(self, user_id_input):
        """
        上传阶段：
        1. GetUserID(user_id_input) → user_orditem_id（验证签字人）
        2. 复用 driver，SaveResult 通知医嘱被执行
        3. PDF 逐页转 JPG，按编号从小到大依次执行：Step6(GetUploadInfo+FTPS上传) → Step7(SaveUploadInfo)
        """
        all_reports = [(p, rep) for p in self.patients for rep in p.reports]
        valid = [(p, rep) for p, rep in all_reports
                 if rep.ord_no and not str(rep.ord_no).startswith("ERR")]
        total = len(valid)
        if total == 0:
        ...
        try:
            ...
            for done, (patient, rep) in enumerate(valid):
                ...
                try:
                    ...
                    # 2b. PDF 转 JPG 列表
                    jpg_files = self._pdf_to_jpg_pages(rep.pdf_path, str(rep.ord_no))
                    if not jpg_files:
                        raise RuntimeError("PDF 转换 JPG 失败")

                    # 2c. JPG 按编号从小到大排序后，依次执行 Step6 → Step7
                    import re as re_module
                    def jpg_sort_key(path):
                        m = re_module.search(r'_(\d+)\.jpg$', os.path.basename(path))
                        return int(m.group(1)) if m else 0
                    jpg_files.sort(key=jpg_sort_key)

                    for jpg_path in jpg_files:
                        jpg_basename = os.path.basename(jpg_path)
                        m = re_module.search(r'_(\d+)\.jpg$', jpg_basename)
                        page_idx = m.group(1) if m else "0"
                        file_path = f"dhcpeftp/images/{rep.ord_no}/{jpg_basename}"

                        # Step 6: GetUploadInfo + FTPS 上传
                        upload_info = _get_upload_info(driver, rep.ord_no, ".pdf", "343")
                        self._ftps_upload_jpg(jpg_path, rep.ord_no)
    ...
    def _pdf_to_jpg_pages(self, pdf_path, ord_no):
        """将多页 PDF 逐页转换为 JPG，命名 {ord_no}_{index}.jpg，返回文件路径列表"""
        import subprocess, glob

        if sys.platform == 'darwin':
            # macOS: 用 sips 转换
            tmp_pdf = f"/tmp/{ord_no}_src.pdf"
            shutil.copy(str(pdf_path), tmp_pdf)

            # sips 只能处理图片，先用 qlmanage 渲染 PDF 页面
            # 1. 渲染每页为 PNG
            pages = []
            try:
                ...
                for i, page in enumerate(reader.pages):
                    ...
                    page_pdf = f"/tmp/{ord_no}_page_{i+1}.pdf"
            ...
            for i, page_pdf in enumerate(pages):
                jpg_out = f"/tmp/{ord_no}_page_{i+1}.jpg"
                try:
                    # 用 sips 转换 PNG → JPG
                    subprocess.run(["sips", "-s", "format", "jpeg", "--out", jpg_out, page_pdf],
                                   timeout=30, check=True)
                    jpg_files.append(jpg_out)
                finally:
                    try:
                        os.remove(page_pdf)
                    except Exception:
            ...
            raise RuntimeError("JPG 转换仅支持 macOS（使用 sips）")

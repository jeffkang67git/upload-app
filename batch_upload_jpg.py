...
def process_single_pdf(pdf_path, user_id, ur_code="SZU01", cur_loc_id=DEFAULT_LOCID, arcim=None):
    ...
    # ── Step 6: GetUploadInfo + FTPS上传（每页JPG重复一次）──────────────────
    # Step 7: SaveUploadInfo（每页JPG重复一次）
    #
    # JPG 命名规则：ord_no_0.jpg, ord_no_1.jpg, ... (从0开始)
    #
    # 多页PDF转JPG后，每页都重复 Step6 + Step7：
    #   GetUploadInfo(ord_no, ".pdf", cur_loc_id)
    #   FTPS上传 page_{i}.jpg → {ord_no}_{i}.jpg
    #   SaveUploadInfo(ord_no, user_internal_id, path, "1")
    #
    # PDF 保持单次上传（ord_no_1.pdf）

    try:
        jpg_paths = pdf_to_jpg_list(pdf_path)
        print(f"      PDF转JPG成功，共 {len(jpg_paths)} 页")

        for i, jpg_path in enumerate(jpg_paths):
            page_idx = i  # JPG 从 0 开始编号
            remote_fname = f"{ord_no}_{page_idx}.jpg"

            # Step 6: GetUploadInfo
            ...
            print(f"      Step6-{i} GetUploadInfo → {upload_resp[:80] if upload_resp else '空响应'}")

            # FTPS 上传
            ftps_upload_file(jpg_path, ord_no, remote_fname)
            print(f"      Step6-{i} FTPS上传 → {remote_fname}")
            ...
            print(f"      Step7-{i} SaveUploadInfo → {save_upload_resp[:80] if save_upload_resp else '空响应'}")

    except RuntimeError as e:
        if "PyMuPDF" in str(e) or "fitz" in str(e):
            return False, f"PDF转JPG失败（PyMuPDF未安装）: {e}"
        raise

    return True, f"上传成功（医嘱号：{ord_no}，共 {len(jpg_paths)} 页JPG）"
